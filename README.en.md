...
- **GitHub Pages publishing**: after every push to main, `repowiki site .` rebuilds and
  publishes; the live sample linked from this README is produced exactly this way.
...
**Live sample**: repowiki's own wiki is published to GitHub Pages —
[open it and see for yourself](https://luomsis.github.io/repowiki/zh/wiki.html)
(rebuilt automatically by the `wiki.yml` workflow above on every push to main).
...
LLM API backends · built-in agent CLI detection/execution · MCP wrappers (the agent-
reads-the-wiki need is met by the static `llms.txt` export) · resident
