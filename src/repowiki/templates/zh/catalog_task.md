...
你的唯一产出是一个 JSON 文件：`{{ROOT}}/state/catalog.json`（相对仓库根目录）。
...
8. （可选）页面节点可设 `archetype` 选择页面骨架，可选值 `module` / `flow` / `layer` / `data` / `api` / `event`（省略等价 `module`）。按页面主题选型，**无法确认时一律用 `module`**；每种非默认原型都有准入条件，且选定后必须在 page_brief 中给出对应证据，否则页面校验会被打回：
   - `flow` 流程型：端到端流程/机制主题（如「一次请求的生命周期」「构建发布流水线」）。准入：确有可完整追踪的起点与终点。证据：page_brief 写明流程的起点与终点。
   - `layer` 分层型：分层体系/调用规约主题（如「分层架构与依赖方向」）。准入：代码确有清晰的层次划分与调用方向约定。证据：page_brief 写明有哪几层。
   - `data` 数据型：数据模型/实体/状态机主题（如「数据模型与存储」「订单状态机」）。准入：确有显式的数据结构与（或）状态迁移。证据：page_brief 写明核心实体。
   - `api` 接口型：对外接口/命令面主题（如「CLI 命令参考」「REST API」）。准入：确有面向使用者的稳定接口面。证据：page_brief 写明接口分组/清单来源。
   - `event` 事件型：事件/消息/发布订阅主题（如「事件总线」「消息流转」）。准入：确有事件或消息机制。证据：page_brief 写明事件清单来源。
   - `module` 结构型（默认）：组件/结构/依赖等其他主题（项目结构/核心组件/架构总览/依赖分析）。
