...
给仓库生成 wiki 的现成方案不少，但各有各的锁：
DeepWiki 一类**云端 AI wiki 服务**，代码要上传出域、按服务付费、产出是黑盒；
Qoder Repo Wiki、ZCode 仓库 Wiki 一类**IDE / 工具内置能力**，好用但锁在单一工具生态——
生成计费（Credits / token）、wiki 或存于本机用户目录（难以进 CI、无版本历史）或依赖厂商平台、
大仓库受限（如 Qoder 单项目上限 1 万文件）；或者让一个 agent 直接通读仓库现写
（大仓库上下文装不下、中断即前功尽弃、难以并行）。
repowiki 走另一条路：**读代码、写 wiki 的智能留给任意 agent，其余一切——任务规划、原子认领、
产出校验、自动修复、断点续跑——做成确定性构建系统。**wiki 以 markdown + mermaid + 源码引用
形态进你的仓库（wiki-as-code），可评审、可增量更新、可被 CI 门禁拦截过期。

| | 云端 AI wiki 服务 | IDE / 工具内置（Qoder、ZCode 等） | 让 agent 直接读仓库 | repowiki |
|---|---|---|---|---|
| 智能来源 | 内置 LLM（不可换） | 工具绑定的模型服务 | 你的 agent（任选） | 你的 agent（任选） |
| 代码出域 | 是（上传托管平台） | 上下文发给所选模型服务 | 否 | 否 |
| 费用 | 按服务付费 | Credits / token 计费 | 视 agent 而定 | repowiki 本身零成本 |
| wiki 归属 | 厂商平台 | 本机用户目录或 `.qoder/` 等，不入库/弱入库 | 一次性产物 | 你的仓库（wiki-as-code）+ 离线站点 |
| CI 门禁 / 增量 | — | 工具内自动刷新为主，无独立门禁 | — | `stale --fail-if-stale` 拦截过期 PR，`update` 按 git diff 增量 |
| 大仓库 | 受服务方配额限制 | 有上限（如 Qoder 1 万文件） | 上下文装不下 | 任务切分，逐页生成 |
| 中断 / 崩溃 | — | 工具内闭环 | 从头再来 | 状态落盘，断点续跑 |
| 并行加速 | — | — | 难协调 | 多 worker 原子认领，天然并行 |
| 产出质量 | 黑盒 | 工具内闭环，无程序化校验 | 靠 agent 自觉 | 模板强制 + 程序化校验 + 自动修复 |
